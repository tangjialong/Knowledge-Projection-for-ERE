...
from base.bert_utils import MultiTaskVAEBertForSequenceClassification
...
def eval_model(model, eval_data_loader, label_list, eval_batch_size, output_dir, device, prefix=""):
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_data_loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            if 'IERE' in prefix:
                logits, KL = model(input_ids, segment_ids, input_mask, labels=None, task='IERE')
            elif 'IDRR' in prefix:
                logits, KL = model(input_ids, segment_ids, input_mask, labels=None, task='IDRR')
    ...
    for epoch_index in trange(int(args.num_train_epochs), desc="Epoch"):
        if IDRR_early_stop < 3:
            ...
            for step_index, batch in enumerate(tqdm(IDRR_train_data_loader, desc="Step")):
                ...
                logits, KL = model(input_ids, segment_ids, input_mask, labels=label_ids, task='IDRR')
                if isinstance(logits, tuple):
                    logits = logits[0]
        ...
        if IERE_early_stop < 3:
            ...
            for step_index, batch in enumerate(tqdm(IERE_train_data_loader, desc="Step")):
                ...
                input_ids, input_mask, segment_ids, label_ids = batch
                # define a new function to compute loss values for both output_modes
                logits, KL = model(input_ids, segment_ids, input_mask, labels=label_ids, task='IERE')
                if isinstance(logits, tuple):
                    logits = logits[0]
...
if __name__ == "__main__":
    ...
    if args.train_mode == 'MultiVAE':
        logger.info("MultiVAE Training ...")
        model = MultiTaskVAEBertForSequenceClassification.from_pretrained(pretrained_model_name_or_path=args.bert_model,
    ...
    if args.do_eval or args.do_train:
        ...
        if args.train_mode == 'MultiVAE':
            logger.info("MultiVAE Training ...")
            model = MultiTaskVAEBertForSequenceClassification.from_pretrained(args.IERE_output_dir,
                                                                              task1_num_labels=IERE_num_labels, task2_num_labels=IDRR_num_labels)
        else:
            raise NotImplementedError('%s not implement' % args.train_mode)
        ...
        if args.train_mode == 'MultiVAE':
            logger.info("MultiVAE Training ...")
            model = MultiTaskVAEBertForSequenceClassification.from_pretrained(args.IDRR_output_dir,
                                                                              task1_num_labels=IERE_num_labels, task2_num_labels=IDRR_num_labels)
        else:
            raise NotImplementedError('%s not implement' % args.train_mode)
