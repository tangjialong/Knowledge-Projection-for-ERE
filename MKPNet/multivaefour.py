...
from base.bert_utils import MultiTaskVAEFourBertForSequenceClassification
...
def eval_model(model, eval_data_loader, label_list, eval_batch_size, output_dir, device, only_four=False, up_bound=False, prefix=""):
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_data_loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            if up_bound:
                if 'IERE' in prefix:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=None, coarselabels=four_ids, task='IERE')
                elif 'IDRR' in prefix:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=None, coarselabels=four_ids, task='IDRR')
            else:
                if 'IERE' in prefix:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=None, coarselabels=None, task='IERE')
                elif 'IDRR' in prefix:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=None, coarselabels=None, task='IDRR')
...
def train_model(model,
    ...
    for epoch_index in trange(int(args.num_train_epochs), desc="Epoch"):
        if IDRR_early_stop < 3:
            # IDRR
            # for step_index, batch in enumerate(IDRR_train_data_loader):
            for step_index, batch in enumerate(tqdm(IDRR_train_data_loader, desc="Step")):
                ...
                if up_bound:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=label_ids, coarselabels=four_ids, task='IDRR')
                else:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=label_ids, coarselabels=None, task='IDRR')
                if isinstance(logits, tuple):
                    logits = logits[0]
                ...
                # logger.info("Step: %d\tloss: %f\tKL: %f" % (step_index, loss, KL))
                loss = loss + lamda_KL * epoch_index * KL
        ...
        if IERE_early_stop < 3:
            # IERE
            # for step_index, batch in enumerate(IERE_train_data_loader):
            for step_index, batch in enumerate(tqdm(IERE_train_data_loader, desc="Step")):
                ...
                # define a new function to compute loss values for both output_modes
                if up_bound:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=label_ids, coarselabels=four_ids, task='IERE')
                else:
                    logits, KL, aux_logits = model(input_ids, segment_ids, input_mask, labels=label_ids, coarselabels=None, task='IERE')
                if isinstance(logits, tuple):
                    logits = logits[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_mode",
                        default='MultiVAEFour',
                        type=str,
                        help="The mode of training. e.g. MultiVAE")
    ...
    if args.train_mode == 'MultiVAEFour':
        logger.info("MultiVAEFour Training ...")
        model = MultiTaskVAEFourBertForSequenceClassification.from_pretrained(pretrained_model_name_or_path=args.bert_model,
    ...
    if args.do_eval or args.do_train:
        ...
        if args.train_mode == 'MultiVAEFour':
            logger.info("MultiVAEFour Training ...")
            model = MultiTaskVAEFourBertForSequenceClassification.from_pretrained(args.IERE_output_dir,
                                                                                  task1_num_labels=IERE_num_labels, task2_num_labels=IDRR_num_labels, coarse_num_labels=4)
        else:
            raise NotImplementedError('%s not implement' % args.train_mode)
        ...
        if args.train_mode == 'MultiVAEFour':
            logger.info("MultiVAEFour Training ...")
            model = MultiTaskVAEFourBertForSequenceClassification.from_pretrained(args.IDRR_output_dir,
                                                                                  task1_num_labels=IERE_num_labels, task2_num_labels=IDRR_num_labels, coarse_num_labels=4)
        else:
            raise NotImplementedError('%s not implement' % args.train_mode)
